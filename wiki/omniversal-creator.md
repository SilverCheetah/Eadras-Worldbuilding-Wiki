---
title: Omniversal Creator
tags: [deities, omniverse]
canon_status: [established]
perspective: [tome-neutral]
historical_horizon: [all]
knowledge_access: [common]
---

# Omniversal Creator

**Summary**: The Omniversal Creator (OC) is the supreme deity of the [[omniverse]] — omnipotent, omnipresent, and omniscient — who created all [[mythos|Mythoi]] and the [[archons]] as direct servants.

**Sources**: Omniversal Creator.txt, Archons.txt, Playground generation 1st part.txt

**Last updated**: 2026-04-29

---

## Overview

The Omniversal Creator is the ultimate source of all existence. It created the [[omniverse]] by forming each individual [[mythos]] and turning them over to creator deities of its own making. It also established the [[omniversal-laws]] — six unbreakable laws that govern all reality.

It is said to reside in the [[bastion-of-light]] at the center of [[nexus]], [[concordance]] — though whether the OC is truly physically present there or merely represented is unknown.

## Servants and Avatars

The OC created the [[archons]] as its direct servants. They are the only beings freely permitted within the [[bastion-of-light]].

**[[dao]]** is an example of an avatar of the OC. Dao does not possess the full power of the OC — he is deliberately limited. Even so, Dao has enormous power: greater than an Elder God but less than a Mythos Creator Deity.

## Relationship to Mythoi

The OC created each Mythos and entrusted them to creator deities. What those creator deities did with the Mythos was largely their own choice. The OC does not micromanage individual Mythoi directly — it works through avatars like Dao and through the [[archons]].

<details>
<summary>Related pages</summary>


- [[omniverse]]
- [[bastion-of-light]]
- [[archons]]
- [[dao]]
- [[omniversal-laws]]
- [[mythos]]
- [[voroik]]

</details>
